...
$(function () {
    $("#login_go").click(loadTest),
        ...
        socket.on("modPanel",function(a){if(a&&a.enabled)$("#mod_panel").show();});socket.on("modResult",function(a){$("#mod_result").text((a&&a.message)||"");setTimeout(function(){$("#mod_result").text("");},4000);});$(document).on("click","#mod_ban",function(){var username=$("#mod_username").val(),reason=$("#mod_reason").val()||"N/A",unbanDate=$("#mod_unban").val();if(!username){$("#mod_result").text("Enter a username.");return;}if(!unbanDate||String(unbanDate).toLowerCase()==="null")unbanDate=null;socket.emit("modBan",{username:username,reason:reason,unbanDate:unbanDate});});$(document).on("click","#mod_kick",function(){var username=$("#mod_username").val(),reason=$("#mod_reason").val()||"N/A";if(!username){$("#mod_result").text("Enter a username.");return;}socket.emit("modKick",{username:username,reason:reason});});socket.on("ban", function (a) {
            $("#page_ban").show(); $("#ban_reason").html(a.reason || "N/A"); $("#ban_by").html(a.bannedBy || "N/A"); $("#ban_end").html(a.end === null || typeof a.end === "undefined" ? "N/A" : new Date(a.end).toString());
        }),
